        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex gap-2 sm:ml-auto">
            <div className="relative">
              ...
              <Input aria-label="Search reports" placeholder="Search reports..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 w-full sm:w-56" />
            </div>
                ...
                {editId === r.id ? (
                  ...
                  <>
                    ...
                    <TableCell className="text-right whitespace-nowrap">
                      <Button size="icon-xs" variant="ghost" aria-label={`Edit report ${r.id}`} onClick={() => startEdit(r)}><Pencil className="w-3 h-3" /></Button>
                      <Dialog><DialogTrigger asChild><Button size="icon-xs" variant="ghost" aria-label={`Delete report ${r.id}`} onClick={() => setDeleteId(r.id)}><Trash2 className="w-3 h-3" /></Button></DialogTrigger>
